...
class OmniscentMIDI {
    constructor() {
        ...
        this.MIDI_TABLE = new Uint8Array([
    ...
    generateMIDI() {
        this.midiData = [];
        let p = 0;
        while(this.MIDI_TABLE[p] - 1 >= 0) {
            const channelData = {};
            channelData.channel = this.MIDI_TABLE[p++] - 1;
            channelData.instrument = this.MIDI_TABLE[p++];
            channelData.data = [];
    ...
    generateMIDILoop(p, channelData) {
        while(this.MIDI_TABLE[p] !== 0x00) {
            if(this.MIDI_TABLE[p] >= 0x80) {
                const count = 0x100 - this.MIDI_TABLE[p++];
                const q = p;
                for(let i = 0; i < count; ++i)
                    p = this.generateMIDILoop(q, channelData);
            } else
                channelData.data.push(this.MIDI_TABLE[p++], this.MIDI_TABLE[p++]);
        }
        ++p;
        return p;
    }
...
class OmniscentModel {
    constructor() {
        ...
        this.INDEX_TABLE = new Uint8Array([
            0x08, 0x28, 0x30, 0x10,
            0x18, 0x38, 0x20, 0x00,
            0x10, 0x30, 0x38, 0x18,
            0x00, 0x20, 0x28, 0x08,
        ...
        this.VERTEX_TABLE = new Int8Array([
            0xE0, 0xE0, 0xE0,
            0x20, 0xE0, 0xE0,
            0x20, 0x20, 0xE0,
            0xE0, 0x20, 0xE0,
        ...
        this.MODEL_TABLE = new Uint8Array([
        ...
        for(let i = 0; i < 4; ++i) {
            ...
            while(true) {
                ...
                for(let j = 0; j < 6; ++j) {
                    ...
                    for(let k = 0; k < 4; ++k) {
                        let index = this.INDEX_TABLE[j * 4 + k] >> 3;
                        let vertexIndex = 0;
                ...
                // 0x0D77
                for(let j = 0; j < 4; ++j) {
                    let index0 = this.INDEX_TABLE[face * 4 + j] >> 3;
                    let index1 = this.INDEX_TABLE[(face * 4 + j) ^ 0x7] >> 3;
                    for(let k = 0; k < 4; ++k)
